...
class UserLoginSerializer(serializers.ModelSerializer):
    email=serializers.EmailField(max_length=232)
    class Meta:
        model=User
...
class UserResetPasswordSerializer(serializers.ModelSerializer):
    password=serializers.CharField(max_length=323,style={'input_type':'password'},write_only=True)
    password2=serializers.CharField(max_length=323,style={'input_type':'password'},write_only=True)
...
class SendPasswordToEmail(serializers.ModelSerializer):
    ...
    def validate(self,attrs):
        ...
        if User.objects.filter(email=email).exists():
            ...
            raise ValidationError('You are not a registation uesr')
        
        
class UserResertPasswordSerializer(serializers.ModelSerializer):
    password=serializers.CharField(max_length=323,style={'input_type':'password'},write_only=True)
    password2=serializers.CharField(max_length=323,style={'input_type':'password'},write_only=True)
    ...
    def validate(self,attrs):
        try:
            ...
            if not PasswordResetTokenGenerator().check_token(user,token):
                raise ValidationError('Token is not valid or exprire')
            user.set_password(password)
            user.save()
            return attrs
